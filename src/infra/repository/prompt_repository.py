...
from src.domain.models.prompt_model import PromptModel
...
class PromptRepository:
    ...
    def create(self, prompt_data: PromptCreateRequest) -> PromptEntity:
        ...
        db_prompt = PromptModel(id=prompt_id, prompt=prompt_data.prompt, created_at=now)
    ...
    def get_by_id(self, prompt_id: str) -> Optional[PromptEntity]:
        db_prompt = (
            self.db.query(PromptModel).filter(PromptModel.id == prompt_id).first()
        )
    ...
    ) -> Optional[PromptEntity]:
        update_dict = update_data.model_dump(exclude_unset=True)

        if update_dict:
            update_dict["updated_at"] = datetime.utcnow()
            self.db.query(PromptModel).filter(PromptModel.id == prompt_id).update(
                update_dict
            )
            self.db.commit()
